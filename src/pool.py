...
def compute_key(tracesTest, features, hamming, sbox, ptTest, meanMatrix, covMatrix, knownkey, ge, final_key, k):
    P_k = np.zeros(256)
...
def pool_atack():
    ...
    # print(covMatrix)
    ge = np.zeros(16)
    final_key = np.zeros(16)

    # For each key bytes
    for k in range(16):
        compute_key(tracesTest, features, hamming, sbox, ptTest, meanMatrix, covMatrix, knownkey, ge, final_key, k)
